...
public class ScreenController extends StackPane {
	...
	public void addScreen(String name, Node window) {
	...
	public boolean setScreen(final String name) {
		if (windows.get(name) != null) { // screen loaded
			...
			if (!getChildren().isEmpty()) { // if there is more than one screen
				Timeline fade = new Timeline(new KeyFrame(Duration.ZERO,
					...
					public void handle(ActionEvent t) {
						...
						getChildren().add(0, windows.get(name)); // add the
																	// screen
				...
				setOpacity(0.0);
				getChildren().add(windows.get(name)); // no one else been
														// displayed, then just
														// show
